...
module.exports = ({ repo, middlewares }, app) => {
  app.post(
    ...
    [
      ...
      middlewares.hasPermissions({
        permissions: [
          { action: 'create', scope: 'any', resource: 'news-category' },
        ],
      }),
    ],
    async (req, res, next) => {
      try {
        ...
        res
          .set('Location', `/news-categories/${category._id}`)
  ...
  app.get('/news-categories', async (req, res, next) => {
    try {
  ...
  app.get('/news-categories/:id', async (req, res, next) => {
    try {
  ...
  app.put(
    ...
    [
      ...
      middlewares.hasPermissions({
        permissions: [
          { action: 'update', scope: 'any', resource: 'news-category' },
        ],
      }),
    ],
    async (req, res, next) => {
  ...
  app.delete(
    ...
    [
      ...
      middlewares.hasPermissions({
        permissions: [
          { action: 'delete', scope: 'any', resource: 'news-category' },
        ],
      }),
    ],
    async (req, res, next) => {
